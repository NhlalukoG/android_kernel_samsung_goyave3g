...
public class SingleByteCharsetProber extends CharsetProber {
    ...
    public float getConfidence() {
        if (this.totalSeqs > 0) {
            float r = 1.0f * this.seqCounters[POSITIVE_CAT] / this.totalSeqs
                    / this.model.getTypicalPositiveRatio();
        ...
        for (int i = offset; i < maxPos; ++i) {
            ...
            if (order < SAMPLE_SIZE) {
                ...
                if (this.lastOrder < SAMPLE_SIZE) {
                    ++this.totalSeqs;
                    if (!this.reversed) {
                        ++(this.seqCounters[this.model.getPrecedence(this.lastOrder * SAMPLE_SIZE
                                + order)]);
                    } else {
                        ++(this.seqCounters[this.model.getPrecedence(order * SAMPLE_SIZE
                                + this.lastOrder)]);
                    }
                }
            }
